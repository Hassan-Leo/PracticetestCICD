...
describe("POST Login API Testing for Super Admin User",() =>{
    ...
    describe("When the authentication is successful for Super Admin User", () => {
        ...
        it("Object contain all the keys requried", (done) => {
            object.body.should.have.keys('status','roles','_id','name','email','register_date','__v','token','RefreshToken');
        ...
        it("To check length of array and objects", (done) => {
                ...
                expect(object.body.email).to.be.eq(api_url.serverurls.raw_data[2].email)
            done();
        });
    });
    after(function(done){
